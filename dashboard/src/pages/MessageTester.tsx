...
import { Send, CheckCircle, XCircle, Loader2, AlertTriangle } from 'lucide-react';
        ...
        <div className="compose-panel">
          ...
          <div className="form-group">
            ...
            {selectedSession && !isSessionReady && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '8px', padding: '8px 12px', borderRadius: '6px', backgroundColor: 'rgba(234, 179, 8, 0.1)', border: '1px solid rgba(234, 179, 8, 0.3)', color: '#EAB308', fontSize: '13px' }}>
                <AlertTriangle size={16} />
                <span>
                  Session is <strong>{selectedSession.status}</strong>.
                  {selectedSession.status === 'qr_ready' && ' Go to Sessions page and scan the QR code with WhatsApp to connect.'}
                  {selectedSession.status === 'disconnected' && ' Go to Sessions page and start the session first.'}
                  {selectedSession.status === 'created' && ' Go to Sessions page and start the session first.'}
                  {selectedSession.status === 'initializing' && ' Session is starting up, please wait...'}
                  {selectedSession.status === 'ready' ? '' : ' Messages can only be sent when status is "ready".'}
                </span>
              </div>
            )}
          </div>
          ...
          <button
            className="send-btn"
            onClick={handleSend}
            disabled={!canWrite || isLoading || !session || !isSessionReady || (recipientType === 'group' ? !selectedGroup : !recipient)}
          >
